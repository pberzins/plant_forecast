...
class PlantForecast():
    ...
    def __init__(self,
                 tiff_files_path='/Users/Berzyy/plant_forecast/data/modis_co/tiff_files/',
    ...
    def set_geometry(self):
        ...
        tl_la_lo, tr_la_lo, br_la_lo, bl_la_lo = self.get_bounding_box(
            q_arr, geo)
    ...
    def load_weather(self, preloaded=True, preloaded_path='preloaded_data/2000_2017_weather.csv'):
        ...
        if preloaded == True and preloaded_path != None:
            ...
            table_list = ['w_00', 'w_01', 'w_02', 'w_03', 'w_04', 'w_05', 'w_06', 'w_07',
                          'w_08', 'w_09', 'w_10', 'w_11', 'w_12', 'w_13', 'w_14', 'w_15', 'w_16', 'w_17']
            ...
            for e in table_list:
                ...
                print(
                    f"Got data from table: {e} in about {time.time()-start} seconds!")
            conn.close()
    ...
    def modis_powerhouse(self, path):
        ...
        quality_file_set = set(list(f for f in os.listdir(
            quality_folder_path) if f.endswith('.' + 'tif')))
        ...
        for f in cross_checked:
            ...
            print(
                f'Compiled Matrix for {date_time} in {time.time()-start} seconds')

        df = pd.DataFrame(np.array(np.array(array_list)),
                          columns=['measurement_date', 'ndvi'])
        df = df.set_index('measurement_date')
        df.index = pd.to_datetime(df.index)
